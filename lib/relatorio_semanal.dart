...
class Relatorio {
  ...
  final DateTime data;
  final String status;
  Relatorio({required this.titulo, this.obs, required this.data, required this.status});
}

class RelatoriosSemanalPage extends StatefulWidget {
  const RelatoriosSemanalPage({super.key});

  @override
  State<RelatoriosSemanalPage> createState() => _RelatoriosSemanalState();
}

class _RelatoriosSemanalState extends State<RelatoriosSemanalPage> {
  ...
  Future<void> buscarDados() async {
    final url = Uri.parse("http://127.0.0.1:8080/relatoriosemanal");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var item in jsonList) {
          ...
          novosRelatorios.add(
            Relatorio(titulo: titulo, obs: obs, data: dataHora, status: status),
          );
        }
